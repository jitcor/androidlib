...
JNIEXPORT void JNICALL Java_unicorn_Unicorn_reg_1write_1mmr
  (JNIEnv *env, jobject self, jint regid, jobject value) {
   ...
   mmr.limit = (uint32_t)(*env)->GetIntField(env, value, fid);

   fid = (*env)->GetFieldID(env, clz, "flags", "I");
   mmr.flags = (uint32_t)(*env)->GetIntField(env, value, fid);

   fid = (*env)->GetFieldID(env, clz, "selector", "S");
   mmr.selector = (uint16_t)(*env)->GetShortField(env, value, fid);
 ...
 */
JNIEXPORT jobject Java_unicorn_Unicorn_reg_1read_1num
